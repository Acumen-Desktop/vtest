...
export enum OnRebuildDo {
  ...
  RestartRenderers = "RestartRenderers",
}
...
export class ElectronForgeVitePlugin extends PluginBase<Config> {
  ...
  getHooks = (): ForgeMultiHookMap => ({
    ...
    prePackage: async (_config, platform, version) => {
      await Promise.all([
        ...this.config.builds.map((info) =>
          vite.build({ configFile: info.configFile })
        ),
        ...this.config.renderers.map((info) =>
          vite.build({ configFile: info.configFile })
        ),
      ]);
    },
  ...
  async viteDevWatcherAndBuilder(
    info: Config["builds"][number],
      ...
      tasks: [
        {
          ...
          task: () =>
            Promise.all(
              this.config.renderers.map(async (info) => {
        ...
        {
          title:
            "Starting main/preload process watchers and builders (config.builds)",
          task: () =>
            Promise.all(
              this.config.builds.map(async (info) => {
                const watcher = await this.viteDevWatcherAndBuilder(
                  info,
                  devServerUrls
                );
